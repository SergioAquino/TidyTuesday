...
## Population growth and fish consumption 
...
## Plot populational and consumption graphs for China, Indonesia, and Japan
...
## Checking population and consumption correlation

```{r}
pop.consumption %>%
  group_by(code) %>%
  #filter(code== "OWID_WRL") %>% 
  summarize(COR=cor(consumption,population)) %>% 
  ggplot(aes(COR)) + 
  geom_vline(xintercept = 0) +
  geom_freqpoly()
```

## Plot Pop Consumption Correlation for Different Countries
